...
select * from parking_log
WHERE check_in <= '2024-06-01 11:00'
AND check_out >= '2024-06-01 10:00';


-- person present between 10:00 to 11:00

SELECT p.person_id, p.name, pl.car_plate, pl.check_in, pl.check_out
FROM parking_log pl
JOIN persons p ON pl.person_id = p.person_id
where pl.check_out between 
'2024-06-01 10:00' and '2024-06-01 11:00';


-- payments checks

select p.name as person_name, pl.car_plate as parking_car,pay.car_plate as pay_for_car
from parking_log pl
join persons p on pl.person_id = p.person_id
left  join payments pay on pl.person_id = pay.person_id
where pl.car_plate != pay.car_plate;
